...
class _ProductListViewState extends State<ProductListView> {
  ...
  Future<List<Product>> fetchCats(http.Client client, page,offset,id,type,keyword) async {
    ...
    final response = await client
        .get(Config.url + 'items?type=' + Provider.of<HomeProvider>(context, listen: false).type.toString()+
        '&sub_id=' +  Provider.of<HomeProvider>(context, listen: false).id.toString()
        +  '&page=' + page.toString()+ '&offset=' + offset.toString()
        +"&type_id="+ type_id.toString()+"&keyword="+keyword.toString());
...
// A function that converts a response body into a List<Product>.
  List<Product> parseCats(responseBody) {
    ...
    Provider.of<ProductsProvider>(context, listen: false).set_releted_products(jsonDecode(responseBody)["data"]["items"]);
  ...
  Future<void> _fetchPage(pageKey) async {
    try {
      final newItems = await fetchCats(
          http.Client(),_pageSize,pageKey, widget.id, widget.type, _searchTerm);
          ...
          InkWell(
            onTap: () {
              SelectDialog.showModal(
                context,
                label: "اختر الفئة",
                items: Provider.of<HomeProvider>(context, listen: false).sub_sub,
                ...
                itemBuilder:
                  ...
                  return Container(
                    ...
                    child: ListTile(
                      onTap: (){
                        var list = Provider.of<HomeProvider>(context, listen: false).sub_sub;
                        if(item["id"] != 0){
                          Provider.of<HomeProvider>(context, listen: false).set_sub_sub_categories(list, item["id"], "subsub");

                        }else{
                          Provider.of<HomeProvider>(context, listen: false).set_sub_sub_categories(list, widget.id, "sub");
                    ...
                    builderDelegate: PagedChildBuilderDelegate<Product>(
                        ...
                        (loading)
                          ...
                          child: Shimmer.fromColors(
                            ...
                            child: ProductsGridView(products:item,num: 1,)

                          ),
                        )
                            :

                        ProductsGridView(products:item,num: 1,)
                    ),
                  ),
